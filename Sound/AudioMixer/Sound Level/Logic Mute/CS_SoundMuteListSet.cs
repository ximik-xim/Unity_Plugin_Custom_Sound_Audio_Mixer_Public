...
/// Устанавливает мут указанным контроллерам уровня звука
/// !! Внимание, этой реализации нужно что бы статус у триггера сохранял кто то из вне !!
///
/// Отличаеться от CS_SoundMuteSet лишь тем, что имеет список ключей для мута(а не 1 ключ)
/// </summary>
public class CS_SoundMuteListSet : MonoBehaviour
{
   [SerializeField]
    ...
    private void Init()
    {
       ...
       // тут не получить взять статус мута(т.к работаю с рядом элементов)
       //_statusMute.SetStatusMuteNotUseEvent(_audioMixerSoundLeve.GetSoundLevelData(_keySoundValue.GetData()).IsMute);
    ...
    private void SetKey()
    {
        ...
        {
            var soundLeve = _audioMixerSoundLevel.GetSoundLevelData(VARIABLE.GetData());
